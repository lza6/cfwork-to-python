# cfwork-to-python
<details>
<summary>cfwork无损让AI大模型转为python本地版本，绕过cfwork子请求50次限制等等一日十万次套餐请求限制</summary>


```
我将提供一个完整的 cfwork的完整 项目源代码（你务必自动识别他的架构和他的目的以及他主要是为了什么等等操作）。你的任务是：
不要简化任何转换UI和功能等等逻辑，禁止出现简化版等等，都要一比一完美完整实现每个功能、逻辑、UI按钮点击事件等等。
核心转换： 将该 cfwork项目的后端代理逻辑，完整、无损地迁移到一个高性能的 python项目中。如果cfwork代码比较多你是否可以复制整体然后呢粘贴为文件之后重命名然后去里面改一改呢？因为总体让你直接转换的话可能耗时比较长，你可以学一学渐进修改递增等等的，比如先了解整体cfwork代码是怎样的干嘛的等等的，然后呢拆分多个小人物渐进式的递增修改加入等等直到完整无损
...
# 如果已经让AI生成完了，担心不够完整以及移植不是无损的丢失什么功能等等的可以用以下提示词：
```
我的原始需要移植的项目如下：


你的目标就是帮我看看我移植完毕的项目是否有丢失功能逻辑啊、UI等等之类的、或者哪里做出对比改变了，我属实不太清楚，需要你帮我列出，这样好能你自行检查是否完整无损移植或者是丢失遗漏某某功能、逻辑、UI等等的，我需要您帮我进行补全并进行优化
你无需一直重构重写文件，因为这样体积太大了，耗时也长，你就查漏补缺、有些没完善的地方完善他，如鉴于需要大量修改没关系的，你就渐进修改比较好，因为这样子你可以节省很多正确的代码无需重复更改，这样进一步增强了你编辑文件代码的能力和效率。最终修改完记得自己再次审查一遍如还是不够完美还需继续这样的操作直到完整完美无损等
